...
import { trackOperation, type ProgressReporter } from '@stores/operationProgressStore'
...
export async function parseFile(filePath: string, ext: string): Promise<string> {
  const name = filePath.includes('/') || filePath.includes('\\') ? filePath.split(/[/\\]/).pop()! : ext.slice(1).toUpperCase()
  return trackOperation('parse', name, async report => {
  report({ stage: 'processing' })
  switch (ext) {
    case '.pdf':
      return parsePDF(filePath, report)
    case '.pptx':
    case '.ppt':
      return parsePPT(filePath, report)
    case '.docx':
 ...
 * 解析 PDF 文件
 */
async function parsePDF(filePath: string, report: ProgressReporter): Promise<string> {
  ...
  report({ current: 0, total: pdf.numPages, unit: 'pages' })
 ...
 */
async function parsePPT(filePath: string, report: ProgressReporter): Promise<string> {
  const w = globalThis as any
  if (!w.electronAPI) throw parseError('parse.apiUnavailable')
  ...
  report({ current: 0, total: slideFiles.length, unit: 'pages' })
